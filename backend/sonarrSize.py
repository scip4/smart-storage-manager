...
class SonarrAPI:
    ...
    def get_episode_files(self) -> List[Dict]:
    ...
    def get_episode_files_by_series(self, series_id: int) -> List[Dict]:
        ...
        try:
            ...
            return []
    def get_episode_files_by_series_all(self, series_list: Optional[List[Dict]] = None) -> List[Dict]:
        """Get episode files for a specific series"""
        try:
    ...
    def get_series_statistics_optimized(self, series_list: Optional[List[Dict]] = None) -> Dict[int, Dict]:
        ...
        try:
            ...
            # Get all episode files at once (this is still the most efficient way)
            print("Fetching episode files...")
            all_files = self.get_episode_files()
    ...
    def get_series_statistics_batch(self, series_ids: List[int] = None, 
        ...
        try:
            ...
            if series_ids:
                ...
                # Get episode files (still need all files as API doesn't support filtering by multiple series)
                all_files = self.get_episode_files()
                
                # Process only files for requested series
                for episode_file in all_files:
                    series_id = episode_file.get('seriesId')
                    if series_id not in series_stats:
                        continue
